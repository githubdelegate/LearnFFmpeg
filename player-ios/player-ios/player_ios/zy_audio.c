...
int prepare_audio(ZYPlayerContext *pCtx){
    pCtx->paudio_stream = pCtx->pformat_ctx->streams[pCtx->audio_stream_index];
    pCtx->paudio_codec_ctx =     pCtx->paudio_stream->codec;
    pCtx->paudio_codec = avcodec_find_decoder(pCtx->paudio_codec_ctx->codec_id);
    if (pCtx->paudio_codec == NULL){
        fprintf(ERR_STREAM, "Couldn't find audio decoder!!!!!!!\n");
        return -1;
    }
    //初始化AVCondecContext，以及进行一些处理工作。
    avcodec_open2(pCtx->paudio_codec_ctx, pCtx->paudio_codec, NULL);
    return 1;
}
...
int audio_decode_frame(ZYPlayerContext *ps);

void audio_callback(void *userdata, uint8_t *stream, int len)
{
    ZYPlayerContext *ps = (ZYPlayerContext *)userdata;
...
int audio_decode_frame(ZYPlayerContext *ps)
{
    ...
    while (packet_queue_get(&ps->audio_packet_queue, &packet) < 0)//队列没有数据
    {
    ...
    dst_nb_samples = av_rescale_rnd(
                                    ...
                                    pframe->sample_rate, pframe->sample_rate, (enum AVRounding)(1));
